...
def rhs_kuramoto_ring(z, K, omega):
    """Rhs of the kuramoto model on ring
    
    dot theta_i = omega_i + K/2 * [sin(theta_i+1 - theta_i) + sin(theta_i-1 - theta_i)]
    ...
    tuple: (dot_x, dot_theta) derivatives
    """
    N = len(z)  # Calculate the number of swarmalators
    ring_graph = nx.cycle_graph(N)
    Z = find_local_order_parameter(z,ring_graph)
    dot_z = omega + (K*Z*np.exp(-1j*z)).imag

    return dot_z

def rhs_kuramoto_network(z, A, K, omega):
    """Rhs of the kuramoto model on ring
    ...
    """
    N = len(z)  # Calculate the number of swarmalators
    # Z = find_local_order_parameter(z,A)
    mat=nx.adjacency_matrix(A)
    mat=mat.toarray()
    Z = np.dot(mat,np.exp(1j*z))
    deg = [degree for node, degree in A.degree()]
    Z=Z/deg
    dot_z = omega + (K*Z*np.exp(-1j*z)).imag
...
def find_twist_order_parameter(z,q):
    N = len(z)
    z1 = [2*np.pi*(i+1)*q/N for i in range(N)]
    Z = np.mean(np.exp(2j*(z+z1)))
    return Z

def find_local_order_parameter(z, A):
    """ Find the regular Kuramoto model sync parameter:
...
def rk4_network(dt, z, A, f, args):
    """Implements one step of the RK4 method."""
    
    # Calculate the four k values
    k1 = dt * f(z, A, *args)
    k2 = dt * f(z + 0.5 * k1, A, *args)
    k3 = dt * f(z + 0.5 * k2, A, *args)
    k4 = dt * f(z + k3, A, *args)
